...
The system SHALL define the following client-side routes using Vue Router: `/` (home/redirect), `/login`, `/challenges`, `/challenges/:id`, `/me`. Navigation between routes SHALL work without a full-page reload.
...
The system SHALL provide a pre-configured Axios instance with `baseURL` set from the `VITE_API_URL` environment variable. All API calls in the application SHALL use this shared instance.
