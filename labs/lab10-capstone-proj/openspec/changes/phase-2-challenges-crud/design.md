...
2. Run `yarn migration:run` against local Postgres; verify in pgAdmin.
...
4. **Rollback**: `yarn migration:revert` drops the table, the indexes, and the enum type in reverse order. The migration's `down()` is generated by TypeORM and we hand-verify it before commit.
